//! 5-input threshold gate: two equivalent syntactic formulations of `next_q`, compared as **syntactic
//! `BoolExpr`** (naive sum-of-products, no canonicalisation) against the **canonical `Bdd`**.
//!
//! The `hold` region is an XOR of two six-term covers; expanding it naively explodes into a huge cube
//! count, while the canonical BDD form stays tiny — the motivation for BDD pre-minimisation before
//! Espresso.
//!
...
fn main() -> std::io::Result<()> {
    println!("5-Input Threshold Gate: Two Equivalent Formulations of next_q");
    println!("============================================================\n");

    // Activation: at least 4 of the 5 inputs high (all 5, then any 4 — 6 product terms).
    ...
    // Deactivation: at most 1 of the 5 inputs high (all low, then any 1 — 6 product terms).
    ...
    // hold / next_q composed through `expr!`, grafting the existing sub-expressions in. These stay
    // syntactic `BoolExpr`: `next_q_v1` and `next_q_v2` are different syntax for the same function.
    ...
    // Stage 1: cube count from the syntactic BoolExpr (naive De Morgan / XOR expansion — the blow-up).
    ...
    println!(
        "  {:<15} {:>12} cubes (sum of all outputs)\n",
        "TOTAL", total_naive
    );
    ...
    // Build a multi-output cover from the canonical BDDs; total is unique cubes (cubes may be shared).
    let mut cover = Cover::new(CoverType::F);
    for (name, bdd) in &bdds {
        cover.add_bdd(bdd, name)?;
    }
    let total_bdd = cover.num_cubes();
    println!(
        "  {:<15} {:>12} cubes (unique cubes in cover)",
        "TOTAL", total_bdd
    );
    println!(
        "  ({:.0}x reduction from naive)\n",
        total_naive as f64 / total_bdd as f64
    );

    // Stage 3: Espresso minimisation.
    println!("Stage 3: Espresso minimisation...");
    ...
    println!(
        "\nConclusion: canonicalising the syntactic BoolExpr into a BDD shrinks the naive {} cubes to {}",
        total_naive, total_bdd
    );
    println!("            (~{}x) before Espresso runs — without it Espresso's input would be intractable.\n",
        total_naive / total_bdd.max(1));

    println!("==============================================================================\n");

    // Equivalence of the two formulations is an O(1) canonical-BDD comparison.
