...
Rust bindings to the UC Berkeley Espresso heuristic logic minimiser. A `build.rs` compiles the vendored C implementation (`espresso-src/`) and generates FFI bindings with bindgen; the Rust crate layers safe, thread-safe, idiomatic APIs on top. Published as `espresso-logic` on crates.io (currently v3.1.x).
