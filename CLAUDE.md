...
- `src/bdd/` — the **canonical BDD layer**, where the semantic operations live. `Bdd<B, C>` (an owned, refcounted `Clone` handle; `handle.rs`) is minted by a `BddBuilder<B, C>` (`builder.rs`) parameterised by two orthogonal type parameters: a sealed `Brand` `B` (`brand.rs`, uniqueness only) and a `ManagerCell` `C` (`LocalCell`/`SyncCell`, storage backend and thread-safety). Builders come from the `bdd_builder!` / `sync_bdd_builder!` macros — there is **no process-global manager**; each builder owns its own. Operations: `&`/`|`/`^`/`!`, `ite`, `restrict`/`cofactor`/`forall`/`exists`, `equivalent_to`, `evaluate`, `to_cubes`/`maximize`, `minimize`, `to_expr`, `fold`, `variables`; `Bdd::builder` recovers a builder onto a stored handle's manager. `scope.rs` adds `BddBuilder::scope`, composing `Copy`, by-reference `ScopedBdd` handles (and `Scope::lift` to splice an owned handle in).
