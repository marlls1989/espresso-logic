...
fn to_minterms_xor_two_vars() {
    ...
    let got: BTreeSet<Minterm<Symbol>> = f.to_minterms(&["a", "b"]).collect();
...
fn to_minterms_widen_with_absent_variable() {
    ...
    let got: BTreeSet<Minterm<Symbol>> = f.to_minterms(&["a", "b", "c"]).collect();
    let want: BTreeSet<Minterm<Symbol>> = [
...
fn to_minterms_true_is_full_cube() {
    ...
    let header = Symbols::new(["a", "b"].iter().map(Symbol::new).collect());
    let got: BTreeSet<Minterm<Symbol>> = t.to_minterms(&["a", "b"]).collect();
    let want: BTreeSet<Minterm<Symbol>> = [
...
fn to_minterms_single_var_splits_other() {
    ...
    let header = Symbols::new(["a", "b"].iter().map(Symbol::new).collect());
    let got: BTreeSet<Minterm<Symbol>> = a.to_minterms(&["a", "b"]).collect();
    let want: BTreeSet<Minterm<Symbol>> = [
        minterm(&header, &[("a", true), ("b", false)]),
        minterm(&header, &[("a", true), ("b", true)]),
    ]
...
fn to_minterms_is_idempotent_and_deterministic() {
    ...
    let once: Vec<_> = f.to_minterms(&["a", "b"]).collect();
    let twice: Vec<_> = f.to_minterms(&["a", "b"]).collect();
...
fn to_minterms_matches_cube_expand_to() {
    ...
    let via_handle: BTreeSet<Minterm<Symbol>> = f.to_minterms(&["a", "b"]).collect();
...
fn build_cover_round_trip() {
    ...
    let from_handle: BTreeSet<Minterm<Symbol>> = f.to_minterms(&["a", "b"]).collect();
...
fn both_context_kinds_agree() {
    ...
    let local_minterms: BTreeSet<Minterm<Symbol>> = lf.to_minterms(&["a", "b", "c"]).collect();
    ...
    let sync_minterms: BTreeSet<Minterm<Symbol>> = sf.to_minterms(&["a", "b", "c"]).collect();
