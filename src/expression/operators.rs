...
// macro, generic over the syntax `S`.
impl_binary_operator!({S: Syntax} BoolExpr<S>, BitAnd, bitand, and);
impl_binary_operator!({S: Syntax} BoolExpr<S>, BitOr, bitor, or);
impl_binary_operator!({S: Syntax} BoolExpr<S>, BitXor, bitxor, xor);

impl<S: Syntax> Not for BoolExpr<S> {
